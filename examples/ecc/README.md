...
cat ./ecc.rb | shasum -a 384 -b | cut -d " " -f1 | ./sign.rb secp384r1 $(cat keypair.json | jq -Mr '.private_key') > signature.json
cat ./ecc.rb | shasum -a 384 -b | cut -d " " -f1 | ./verify.rb secp384r1 $(cat keypair.json | jq -Mr '.public_key') $(cat signature.json | jq -Mr '.signature')
